...
export default function JobsPage() {
    ...
    const {data:jobsSkillsData, loading:loadingJobSkills} = useQuery(JobSkills);
    const {data:jobRequirementsData, loading:loadingJobRequirements} = useQuery(JobRequirements);
    const {data:jobBenefitsData, loading:loadingJobBenefits} = useQuery(JobBenefits);
    ...
    return (
        <PageLayout title="Jobs  page" userType={"user"}>
            {jobsData&&jobsSkillsData&&jobRequirementsData&&jobBenefitsData&&
